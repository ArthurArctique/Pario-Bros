...
screen = pygame.display.set_mode((tailleecr[0]*0.9, tailleecr[1]*0.9),RESIZABLE)
fullscreen = False
...
while 1+1 == 2:
    ...
    if fullscreen:
        screen = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)
    bluewall2 = pygame.transform.scale(bluewall,(imgsize[0]*diff2[0],imgsize[1]*diff2[1]))
    screen.fill((0,0,0))
    screen.blit(bluewall2,(centre[0]-((imgsize[0]*diff2[0])/2),centre[1]-((imgsize[1]*diff2[0])/2)))
    pygame.display.flip()
    for event in pygame.event.get():
        if event.type == VIDEORESIZE and not fullscreen:
            ...
            if diff[1] != 1 and diff[0] == 1:
                ...
                diff = (diff[1],diff[1])
                diff2 = (diff2[0]*diff[1],diff2[1]*diff[1])
            elif diff[0] != 1 and diff[1] == 1:
                x = (diff[0]*x[0],diff[0]*x[1])
                diff = (diff[0],diff[0])
                diff2 = (diff2[0]*diff[0],diff2[1]*diff[0])
