...
class Mobs:
    ...
    def freeze(self):
        nb_joueur = jeu.classDict['monde'].players
        for i in nb_joueur:
            joueur = i
        
        if self.pos[0] < int(nb_joueur[joueur].pos[0]-1500) and self.pos[0] > 0  or self.pos[0] > int(nb_joueur[joueur].pos[0]+1500):
            self.speed = 0
...
class World:
    ...
    def monstres(self):#fais apparaitre les monstres si proche du joueur sinon le rajoute a une liste d'attente
        for name in self.blockRECT:
            if self.blockRECT[name][0][2][10]:
                for i in self.players:
                    joueur = i
                identifiant = self.blockRECT[name]
                for i in range(len(self.blockRECT[name])):
                        
                    if int(self.players[joueur].pos[0]+1200) < self.blockRECT[name][i][0][0]:
                        self.monstre_spawn.append((identifiant[0][2][4],(identifiant[i][0][0],identifiant[i][0][1])))
                    
                    else:
                        self.monstre[identifiant[0][2][4]+str(self.nb_monstre)] = Mobs(self.screen,self.blockRECT,identifiant[0][2][4],(identifiant[i][0][0],identifiant[i][0][1]),identifiant[0][2][4]+str(self.nb_monstre))
                        self.nb_monstre+=1
    ...
    def spawn(self):# fais apparaitre les monstres quand ils sont a une distance raisonnable du joueur
        ...
        if self.monstre_spawn != []:
            liste_supp = []
            nb_joueur = self.players
            for x in nb_joueur:
                nom_perso = x
            
            for i in self.monstre_spawn:
                if int(self.players[nom_perso].pos[0]+1200) > i[1][0]:
